    ...
    public class ProductImportService : IImportService<Product>
    {
        ...
        public async Task ImportFromStreamAsync(Stream stream, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Validate headers
                var headers = worksheet.Row(1).Cells(1, 5).Select(c => c.GetString().Trim()).ToList();
                var expectedHeaders = new[] { "Категорія", "Назва", "Характеристики", "Кількість", "Ціна" };
                ...
                while (!worksheet.Row(row).IsEmpty())
                {
                    try
                    {
                        ...
                        var characteristics = worksheet.Cell(row, 3).GetString()?.Trim();
                        var quantityStr = worksheet.Cell(row, 4).GetString()?.Trim();
                        var priceStr = worksheet.Cell(row, 5).GetString()?.Trim();
